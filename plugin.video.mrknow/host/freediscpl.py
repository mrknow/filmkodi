...
scriptname = "Filmy online www.mrknow.pl - freediscpl"
...
class freediscpl:
    def __init__(self):
        log.info('Starting freediscpl.pl')
    ...
    def listsMainMenu(self, table):
        for num, val in table.items():
            self.add('freediscpl', 'main-menu', val, 'None', 'None', 'None', 'None', 'None', True, False)
    ...
    def listsCategoriesMenu(self):
        ...
        if linki_ost:
            ...
            for mylink in linki_all:
                ...
                self.add('freediscpl', 'categories-menu', mylink.text,mylink.text,  'None', catUrl + 'kategoria['+mylink['data-id']+']+', 'None', 'None', True, False,str(1),mylink['data-id'])
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsOther(self, url):
            ...
            if len(match1) > 0:
                for i in range(len(match1)):
                        ...
                        self.add('freediscpl', 'playSelectedMovie', 'None', match1[i][1], 'None', match1[i][0], 'aaaa', 'None', False, True)
    ...
    def listsItems(self, url,strona='', kategoria=''):
        ...
        if linki_ost:
            ...
            for mylink in linki_ost:
                ...
                #add(self, service, name,               category, title,     iconimage, url, desc, rating, folder = True, isPlayable = True):
                self.add('freediscpl', 'playSelectedMovie', 'None', mylink.h3.text, mylink.img['src'], mylink.a['href'], 'aaaa', 'None', False, True)
        # add(self, service, name,                   category,        title,       iconimage,          url,                           desc, rating, folder = True, isPlayable = True,strona=''):
        self.add('freediscpl', 'categories-menu', 'Następna strona','Następna strona',  'None', myurl, 'None', 'None', True, False,str(nowastrona),kategoria)
    ...
    def listsItemsA(self, url):
        ...
        if len(match) > 0:
            for i in range(len(match)):
                self.add('freediscpl', 'page-menu', 'None',  match[i],  'None', mainUrl, 'aaaa', 'None', True, False)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsS(self, url, strona):
        ...
        if len(match0) > 0:
            for i in range(len(match0)):
                ...
                self.add('freediscpl', 'items-menu', 'None', self.cm.html_special_chars(title),  'None', match0[i][0], 'aaaa', 'None', True, False)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    def listsItemsOst(self, url):
        ...
        if linki_ost:
            ...
            for mylink in linki_all:
                ...
                self.add('freediscpl', 'playSelectedMovie', 'None', mytitle ,  myimage, myhref, 'aaaa', 'None', False, True,'')
    ...
    def listsItemsTop(self, url):
        ...
        if len(match1) > 0:
            for i in range(len(match1)):
                #print ("M",match1[i])
                self.add('freediscpl', 'playSelectedMovie', 'None', self.cm.html_special_chars(match1[i][1].strip() + ' ' + match1[i][3].strip() + ' ' + match1[i][4].strip()),  'None', mainUrl[:-1]+ match1[i][2], 'aaaa', 'None', False, False,'')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
